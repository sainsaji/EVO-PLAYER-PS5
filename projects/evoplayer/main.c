...
/* EVO: source-over blend for the anti-aliased UI primitives.
 *
 * Framebuffer pixels are ABGR8888 (0xAABBGGRR), matching the format handed to
 * sceVideoOutSetBufferAttribute2. Duplicated rather than shared with
 * prospero_osd_blend_pixel because that one is defined ~3000 lines further
 * down, after the text renderer that needs this. */
static inline uint32_t evo_blend_pixel(uint32_t dst, uint32_t src, int alpha)
{
...
static void prospero_settings_load(void)
{
    ...
    if (file) {
        ...
        /* Accept 5 (pre-EVO), 6 (pre-theme) or 7. Fewer is corrupt. */
        if (values_read < 5) {
            loaded_profile = 0;
            loaded_resume = 1;
            loaded_view = 0;
...
static void rr_icon(uint32_t *fb,int x,int y,int idx){
    /* EVO: icons take the theme accent, see rr_img_tint(). */
    const uint32_t tint=evo_theme_current()->accent;
    switch(idx){case 0:rr_img_tint(fb,x,y,EVO_ICON_BROWSE_USB_W,EVO_ICON_BROWSE_USB_H,EVO_ICON_BROWSE_USB,tint);break;case 1:rr_img_tint(fb,x,y,EVO_ICON_RECENT_FILES_W,EVO_ICON_RECENT_FILES_H,EVO_ICON_RECENT_FILES,tint);break;case 2:rr_img_tint(fb,x,y,EVO_ICON_FAVORITES_W,EVO_ICON_FAVORITES_H,EVO_ICON_FAVORITES,tint);break;case 3:rr_img_tint(fb,x,y,EVO_ICON_SETTINGS_W,EVO_ICON_SETTINGS_H,EVO_ICON_SETTINGS,tint);break;case 4:rr_img_tint(fb,x,y,EVO_ICON_DEVELOPER_TOOLS_W,EVO_ICON_DEVELOPER_TOOLS_H,EVO_ICON_DEVELOPER_TOOLS,tint);break;case 5:rr_img_tint(fb,x,y,EVO_ICON_ABOUT_SUPPORT_W,EVO_ICON_ABOUT_SUPPORT_H,EVO_ICON_ABOUT_SUPPORT,tint);break;case 6:rr_img_tint(fb,x,y,EVO_ICON_CHEVRON_W,EVO_ICON_CHEVRON_H,EVO_ICON_CHEVRON,tint);break;
    /* EVO: settings-row icons, see tools/gen_icons.py */
