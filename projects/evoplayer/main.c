...
/* GL-5 (#81): the bitmap-font atlases (renderer_reset_assets.h, evo_font_punct.h),
 * the SDF icon table (evo_icons.h) and evo_font.h are deleted - every screen and
 * overlay now renders text through RmlUi (FreeType). */
...
/* Colour packing to the 0xAABBGGRR framebuffer layout (kept - not a font thing;
 * used by the swscale RGBA path and a few remaining fills). */
#ifndef RR_BGRA
...
double g_gl_present_ms = 0.0;   /* #63: smoothed blit+swap cost, player present */
...
/* GL-5 (#81): the bitmap glyph tables are gone, so the C-side line wrapper uses
 * a conservative per-face average advance. RmlUi lays the wrapped line out
 * itself with FreeType metrics; this only decides where to break, and a slight
 * underestimate just yields marginally short lines rather than clipped ones. */
static int reader_measure(const char *s, int len, int face)
{
    int adv = (face == EVO_FACE_SMALL) ? 9
            : (face == EVO_FACE_SUB)   ? 12
            : (face == EVO_FACE_MENU)  ? 16 : 21;
    (void)s;
    return len * adv;
}
...
/*
 * #81 / #63: build the playback diagnostic HUD payload and push it to the RmlUi
 * #stats-hud. Called from draw_player_screen while show_stats_for_nerds is on,
 * just before the OSD render. Replaces the old immediate-mode rr_text panel.
 */
static void prospero_feed_perf_hud(void)
{
    ...
    int vw = current_metadata.width, vh = current_metadata.height;
    int ach = 0, arate = detected_audio_rate;

    if (play_fmt && video_stream_index >= 0 &&
        video_stream_index < (int)play_fmt->nb_streams) {
        AVCodecParameters *cp = play_fmt->streams[video_stream_index]->codecpar;
        if (cp) {
            snprintf(vcodec, sizeof vcodec, "%s", avcodec_get_name(cp->codec_id));
            vw = cp->width; vh = cp->height;
        }
    }
    if (play_fmt && audio_stream_index >= 0 &&
        audio_stream_index < (int)play_fmt->nb_streams) {
        AVStream *st = play_fmt->streams[audio_stream_index];
        if (st && st->codecpar) {
            snprintf(acodec, sizeof acodec, "%s", avcodec_get_name(st->codecpar->codec_id));
            ach = st->codecpar->ch_layout.nb_channels;
            arate = st->codecpar->sample_rate;
            const AVDictionaryEntry *lang = av_dict_get(st->metadata, "language", NULL, 0);
            if (lang && lang->value && lang->value[0])
                snprintf(alang, sizeof alang, "%s", lang->value);
        }
    }
    ...
    snprintf(l_video, sizeof l_video, "VIDEO  %s  %dx%d  %.3f fps  (%s)",
             vcodec, vw, vh, evo_pb_video_fps(), backend);
    snprintf(l_audio, sizeof l_audio, "AUDIO  %s  %s  %dch  %d Hz",
             acodec, alang, ach, arate);
    snprintf(l_subs, sizeof l_subs, "SUBS  %s  /  %s",
    ...
    snprintf(l_perf, sizeof l_perf, "RENDER %d fps  /  DECODE %d fps",
             perf_render_fps, perf_decode_fps);
    snprintf(l_queues, sizeof l_queues, "QUEUES  video %d/%d  audio %d/%d  pcm %d",
             packet_queue_count(&video_packet_queue), video_packet_cap,
             packet_queue_count(&audio_packet_queue), audio_packet_cap,
             audio_queue_count);
    snprintf(l_clocks, sizeof l_clocks, "CLOCKS  video %.2f  audio %.2f  delta %.3f",
    ...
    hud.line_video = l_video;   hud.line_audio = l_audio;
    hud.line_subs = l_subs;     hud.line_perf = l_perf;
    hud.line_queues = l_queues; hud.line_clocks = l_clocks;
    hud.hist_len = EVO_PERF_HIST;
    hud.gpu_hist = s.gpu_hist;  hud.ram_hist = s.ram_hist;  hud.cpu_hist = s.cpu_hist;
    hud.gpu_pct = s.gpu_pct;    hud.gpu_peak_pct = s.gpu_peak_pct;
    hud.ram_mb = s.ram_mb;      hud.ram_peak_mb = s.ram_peak_mb;
    hud.ram_total_mb = s.ram_total_mb;
    hud.cpu_pct = s.cpu_pct;    hud.cpu_peak_pct = s.cpu_peak_pct;
...
void draw_player_screen(uint32_t *fb) {
    ...
    if (g_k4_osd_publish) {
        ...
        /* #81: the NOW PLAYING visualiser is #music-view in playback.rml now
         * (fed via evo_playback_osd_params_t.music_mode below). Just lay down an
         * opaque ground for the OSD document to composite onto. */
        for (int i = 0; i < WIDTH * HEIGHT; i++) {
            fb[i] = 0xFF000000;
        }
        (void)music_pulse;
...
double percentage = 0.0;
    ...
    /*
     * #81: the active subtitle cue, resolved once here and handed to the RmlUi
     * OSD (Unicode caption overlay) instead of the old ASCII bitmap
     * prospero_subtitle_draw(). Same clock basis as before: raw audio/video
     * clock + resume offset, minus the user delay.
     */
    ...
    if (prospero_subtitle_enabled && !prospero_music_mode) {
        ...
        if (prospero_subtitle_use_external) {
            const ProsperoSubtitleCue *cue = prospero_subtitle_active_cue(sub_pos);
            if (cue && cue->text[0])
                snprintf(active_sub_text, sizeof(active_sub_text), "%s", cue->text);
        } else {
    ...
    if (osd_visibility > 0) {
        ...
        p.alpha = alpha;
        p.subtitle_text = active_sub_text[0] ? active_sub_text : NULL;
        p.subtitle_face = prospero_subtitle_face;
        p.subtitle_raised = 1;
        p.chrome_hidden = 0;
        p.fps = perf_render_fps;
        p.debug_overlay = show_debug_overlay;
        p.music_mode = prospero_music_mode;
        p.music_codec = (audio_ctx && audio_ctx->codec && audio_ctx->codec->name)
                            ? audio_ctx->codec->name : "AUDIO";

        if (evo_rmlui_is_initialized()) {
            evo_rmlui_update_playback_params(&p);
            if (show_stats_for_nerds)
                prospero_feed_perf_hud();
            evo_rmlui_render_playback_osd(fb, WIDTH, HEIGHT);
        }
    } else if ((active_sub_text[0] || show_debug_overlay) &&
               !prospero_music_mode && evo_rmlui_is_initialized()) {
        /*
         * #81: controls faded but a caption and/or the dev FPS pill is up —
         * render the OSD document with the chrome suppressed so only
         * #subtitle-box / #fps-pill composite over the video quad. (The present
         * block keeps gl_scratch alive while _subs_live / show_debug_overlay and
         * hashes it so a static overlay costs one composite.)
         */
        evo_playback_osd_params_t p;
        memset(&p, 0, sizeof(p));
        p.title = title[0] ? title : "Video Playback";
...
skip_screen_input:
        ...
        if (screen == 0)
            ...
            /* #81: full-screen image viewer is an RmlUi screen now. Decode
             * (load_bmp_file / load_stb_image_file) still fills bmp_pixels;
             * RmlUi draws it as a `contain`-fit memory-texture quad. */
            evo_rmlui_image_params_t _ip;
            memset(&_ip, 0, sizeof(_ip));
            _ip.title  = current_image[0] ? current_image : current_image_path;
            _ip.pixels = bmp_pixels;
            _ip.w      = bmp_w;
            _ip.h      = bmp_h;
            _ip.loaded = bmp_loaded;
            evo_rmlui_update_image(&_ip);
            evo_rmlui_render_image(linear, WIDTH, HEIGHT);
        }
        else if (screen == 4)
        ...
        /* Virtual Keyboard modal overlay (#81: RmlUi doc in its own context).
         * Called every frame like the toast so a close hides the document -
         * evo_screen_keyboard() is a cheap no-op / hide when g_kb is closed. */
        ...
        if (gl_active) {
            ...
            if (_video) {
                ...
                if (_osd_active || _osd_was) {
                    ...
                    g_k4_osd_publish = 0;
                    /* Redraw the toast onto the scratch AFTER draw_player_screen's
                     * memset — the dispatch drew it earlier in the frame but that
                     * clear wiped it. Cheap: a no-op unless a toast is up. (The
        ...
        evo_perf_monitor_tick(screen == SCREEN_PLAYER ? g_gl_present_ms : 0.0);
