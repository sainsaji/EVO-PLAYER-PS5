...
#ifdef EVO_AUTOPLAY_HDR
#include "evo_autoplay.h"     /* generated by package-app.sh --autoplay: EVO_AUTOPLAY_FILE */
#endif
...
#ifdef EVO_AUTOPLAY_FILE
        /*
         * Unattended decode test (scripts/package-app.sh --autoplay <path>):
         * open <path> directly ~4 s after the menu (no controller), then write
         * a once-a-second status line to /mnt/usb0/evo_autoplay_status.txt so
         * tools/vdec-test.sh can tell "playing" from "hung after frame 1".
         * Quits the app after EVO_AUTOPLAY_QUIT_S seconds so the next relaunch
         * is clean.  Breadcrumbs also in /mnt/usb0/{evo_vdec.log,pp_4k_stage_*}.
         */
        {
            static long long s_ap_t0 = 0, s_ap_last = 0;
            static int s_ap_opened = 0, s_ap_stopped = 0;
            long long ap_now = now_ms();
            if (!s_ap_t0) s_ap_t0 = ap_now;
            long long ap_el = ap_now - s_ap_t0;

            if (!s_ap_opened && ap_el > 4000) {
                s_ap_opened = 1;
                /* sandbox already open from evo_jailbreak_self() at boot — the
                 * USB log files prove it; do NOT call evo_jailbreak_ensure()
                 * here (it blocks the render thread ~1.5s mid VO-reconfig). */
#ifdef EVO_HAVE_BUILD_ID
                pp_stage_bc("AUTOPLAY_BUILD", EVO_BUILD_ID);
#endif
                pp_stage_bc("AUTOPLAY_OPEN", EVO_AUTOPLAY_FILE);
                snprintf(current_media_path, sizeof(current_media_path), "%s",
                         EVO_AUTOPLAY_FILE);
                requested_resume_seek_pos  = 0.0;
                resume_base_offset_seconds = 0.0;
                evo_nav_push(EVO_SCREEN_PLAYER);
            ...
            if (s_ap_opened && ap_now - s_ap_last > 1000) {
                s_ap_last = ap_now;
                int done = (evo_pb_is_eof() || evo_pb_decode_fatal() ||
                            (ap_el > 8000 && !evo_pb_is_active()));
                FILE *sf = fopen("/mnt/usb0/evo_autoplay_status.txt", "w");
                if (sf) {
#ifdef EVO_HAVE_BUILD_ID
                    fprintf(sf, "build=%s\n", EVO_BUILD_ID);
#endif
                    fprintf(sf, "t=%lld scr=%d be=%d pos=%.2f dur=%.1f fps=%.1f "
                                "fatal=%d eof=%d active=%d%s\n",
                            ap_el / 1000, screen, evo_pb_active_backend(),
                            evo_pb_position_s(), evo_pb_duration_s(),
                            evo_pb_video_fps(), evo_pb_decode_fatal(),
                            evo_pb_is_eof(), evo_pb_is_active(),
                            done ? "  DONE" : "");
                    fclose(sf);
                }
                /* Natural end: leave the app AT the menu — no _exit (that can
                 * wedge the app slot). PS-button closes it for the next run. */
                if (done && !s_ap_stopped) {
                    s_ap_stopped = 1;
                    pp_stage_bc("AUTOPLAY_END", evo_pb_decode_fatal() ? "fatal"
                                : evo_pb_is_eof() ? "eof" : "inactive");
                }
            }
        }
#endif
