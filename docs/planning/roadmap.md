  ...
  positional PRX import stubs ─┬─► #27 GPU Step 2: sceAgc convert+present ─ ✅ CLOSED
   (package-app.sh step 6b,      │   2026-09-04, PR #61 — GTA 4K on GPU, correct colour,
    unconditional DT_NEEDED)      │   982µs/frame. Leftovers: #62 A/B, #37 row, #41 (HEVC+VP9 ✅ verified)
                                  ...
                                  │   (#34 native IME kb ✅ CLOSED 2026-09-22 — libSceCommonDialog stubbed)
...
| **#34** ✅ CLOSED 2026-09-22 | ~~App module crashes when the native PS5 IME keyboard is opened (directory search).~~ Fixed: `libSceImeDialog` + `libSceCommonDialog` are now linked/PRX-stubbed for `PPSA99039` (`libSceCommonDialog` is a positional `DT_NEEDED` from `tools/native-app/stubs/prx/*.syms` — see tooling.md), so the native IME keyboard opens from the app module; the virtual keyboard remains as the Settings → Interface option | `projects/evoplayer/ui/src/evo_keyboard.c`, `ui/include/evo_ime_dialog.h`, `main.c` (~L12504 search, ~L7581 kb toggle), `scripts/package-app.sh` (steps 5 + 6b), `tools/native-app/stubs/prx/README.md` |
