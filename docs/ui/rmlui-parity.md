...
| Subtitle track picker | `subtitles` | OK / HW | Track list over live video, size cycle, preview line. Long track labels fit. The scroll window is 6 rows, matching the document; it used to be 8, so the focus could land on rows that were never drawn. **#102 AUTO-SYNC row** (below EXTERNAL SRT, `.row-action`: no radio): second row (after OFF) whenever a track exists; dimmed (`.row-disabled`) unless the active track - external SRT or embedded text track - is on a local file. Embedded tracks get an offset only, no framerate ratio. The badge shows live `ANALYSING... N%`, and the `SYNC:` pill appends the ratio after a framerate fix. Host fixtures: `rml_subtitles_autosync_{disabled,idle,analysing,done}`. hw-verify pending. |
