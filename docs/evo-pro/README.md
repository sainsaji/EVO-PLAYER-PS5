...
| [gpu-rendering-plan.md](gpu-rendering-plan.md) | Move YUV convert + composite + UI off the CPU onto `sceAgc` — the fix for the ~11 fps RmlUi frame. References: `third_party/ProsperoLight/src/native_agc_present.cpp` + [sharpprospero-agc-reference.md](sharpprospero-agc-reference.md) | 📝 planned, after Phase 1b m1 |
