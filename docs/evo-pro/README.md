...
| [phase-1b-app-module.md](phase-1b-app-module.md) | **Phase 1b** — repackage EVO as app module `PPSA99039` (fork the `ps5-native-app-boilerplate` build tail, clean-room `libc.prx`, ShadowMountPlus). **Milestone 1:** the unchanged FFmpeg-software player booting to the RmlUi menu in the app sandbox | 🟢 **tasks 1–5 done on hardware 2026-09-02** — boots to menu, pad works; tasks 6–8 remain |
| [gpu-rendering-plan.md](gpu-rendering-plan.md) | Move YUV convert + composite + UI off the CPU onto `sceAgc` — the fix for the ~11 fps RmlUi frame. Reference: `third_party/ProsperoLight/src/native_agc_present.cpp` | 📝 planned, after Phase 1b m1 |
