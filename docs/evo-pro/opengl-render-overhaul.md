...
| **GL-3** (#79) | GL owns `sceVideoOut` for the **whole session** on device (single owner — `pp_videoout` / `pp_agc` present are removed, not run alongside). Menu screens render + swap through GL. **The player frame's *present* moves here too** — a GL texture blit of `pp/`'s already-CPU-converted RGBA frame + `eglSwapBuffers` — because there is no second flip queue for it. `pp/` keeps decode + the CPU convert + clock + seek. Delete the `m_surface` / `AgcGeoPresent` dual path + `#28` geo sink; retire present routes #2/#3; kill the `#32` scrub-overlay machine; fold in `#49`. Staged B1–B5 — see the [#79 alignment comment](https://github.com/sainsaji/EVO-PLAYER-PS5/issues/79). **B1 DONE + hw-verified 2026-09-09:** `scripts/package-app.sh --gl` links ps5-opengl and cuts `main()` over to a persistent `evo_gl_context_device.cpp` (EGL window surface, GL 3.3 Core) in place of `pp_agc_init` + the `PP_BACKEND` present path; the B1 boot loop `glClear`s teal + `eglSwapBuffers`. Receipt: `GL ctx: current - PS5 homebrew / PS5 AGC / GL 3.3 (Core Profile) Mesa 26.2.0 / GLSL 3.30`, teal on the TV, stays up, resident native decoder still inits fine before GL. | Every non-player screen + the player play through GL on hardware; `#49` items 1–5 landed; `uiview.sh --all` unchanged. |
| **GL-4** (#80) | Make the video path efficient. Replace `pp/`'s CPU YUV→RGB with a GLSL fragment shader sampling NV12/P010 directly on the fullscreen quad (GL-3 already put the pixels on screen via GL). Delete `pp_converter_fused/_parallel`, `pp_compute_pipeline`, `tile_copy`, the 5-way dispatch, the V8/V3/1080 backend enum. Aspect-ratio math into the vertex quad (closes `#76`). | GTA 4K + 1080p through the GLSL path; `#62` parity vs a reference frame; `#76` fixed; CPU converters + `tile_copy` + the backend enum gone. |
