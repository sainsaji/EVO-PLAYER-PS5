...
| **GL-4** (#80) | Make the video path efficient, then delete what it replaced. **Stage 1 (G47->G55 bump + `--gl-smoke` upload re-measure) hw-verified 2026-09-10:** the `glTexSubImage2D` staging-copy wall is **RGBA8-only** - R8 / RG8 uploads are free (1080p luma 0.10 ms, chroma 0.03 ms; 4K 0.90 / 0.46 ms), so an NV12 two-plane upload is 0.13 ms/frame (1080p) / 1.36 ms/frame (4K) and **no zero-copy import was needed**. **Stage 2a-c hw-verified 2026-09-10:** zero-copy NV12 (decoder planes -> R8+RG8 textures, no copy) + GLSL YUV->RGB (BT.601 limited, `.bgr`-swizzled for the BGRA scanout) + aspect as a vertex-quad scale (closes `#76`) + OSD composited over the video, sample-hash-gated. GTA 4K native + 1080p smooth (`late_drop` 0-1), `blit~10-13ms` (was ~65). **Stage 2d + Stage 3 code-complete 2026-09-10, hw-verify-pending:** the seek-discard window now serves a frozen snapshot of the last published frame (`pp_playback` `hold_buf`, one copy per seek) with the scrub OSD held up over it, replacing the `#32` overlay machine outright; and `pp_converter*.c`, `pp_compute_pipeline.c`, `tile_copy.c`, `pp_product_path.h` + the 4K stage-ladder headers, `bench.sh` / `bench_converter.c`, `pp_videoout`'s CPU present, the 5-way present dispatch, the backend enum, the 4K VO reconfigure machinery and `--no-gl` are all deleted. `#62` became an exhaustive host parity sweep (`tools/gl_yuv_parity.py`, 99.390% bit-exact, max delta 1/255) which caught a real 16/256-vs-16/255 offset error in the shader. Full plan: [gl4-video-path-plan.md](gl4-video-path-plan.md). | GTA 4K + 1080p through the GL video path real-time; `#62` parity in [validation.md](../validation.md); `#76` fixed; CPU converters + `tile_copy` + the backend enum gone. |
...
| `#76` (aspect-ratio cycle corrupts 4K) | **Fixed by GL-4** - aspect is a vertex-quad scale; `force_v3_fallback` and the linear-vs-tiled attr mismatch are deleted |
