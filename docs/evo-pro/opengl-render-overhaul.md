...
> **Status (2026-09-10): GL-1 … GL-6 DONE.** The overhaul is complete on
> `refactor/main-c-media-modules`. Everything renders through one `ps5-opengl`
> GL/EGL context (Mesa 26.2, GL 3.3 Core, PS5 AGC/VideoOut backend), brought up
...
Deleted across GL-4 / GL-5 / GL-6:

- **GL-4:** `pp_converter_fused.c`, `pp_converter_parallel.c`,
  `pp_compute_pipeline.c` (misnamed — CPU SIMD), `tile_copy.c`, the
  `PP_BACKEND_4K_V8_FUSED` / `_V3_FALLBACK` / `_1080_STANDARD` enum + 5-way
  present dispatch, the 4K VO reconfigure machinery, the `#32`
  `prospero_scrub_ovl_state` machine, `bench.sh`.
- **GL-5:** `draw_char` / `draw_text` + `evo_draw.c` bitmap font, `evo_widgets.c`,
  `evo_font_charset.h`, `evo_keyboard.c` immediate mode, `pp_map_yuv420p10_to_8`.
- **GL-6:** `pp_agc.c`, `pp_agc_osd.c`, `pp_videoout.c`, `pp_platform.h`,
  `evo_rmlui_render_agc.cpp` + the `#28` geo-sink hooks, the vendored shader
  blobs (`pp/blobs/`, `pp/shaders/`, `agc_blobs.S`, `agc_ui_blobs.S`),
  `tools/build-shader.sh`.

**Still standing:** `evo_rmlui_render.cpp` (CPU coverage rasteriser + clip-mask)
and its `m_surface` cache — it is the live device menu path (CPU-raster → one GL
blit); RmlUi-native GL3 is kept beside it behind `/mnt/usb0/evo_gl_rmlui`.
`pp_product_overlay_enter` / `_leave` survive as bare presentation-clock
suspend/resume helpers for modal overlays (Media Info, stop prompt).
...
| **GL-6** (#82) ✅ **DONE 2026-09-10** | Cutover + demolition. Deleted `pp_agc.c` present/geo/osd, `pp_agc_osd.c`, `pp_videoout.c`, `pp_platform.h`, `evo_rmlui_render_agc.cpp` + the `#28` geo sink hooks in `evo_rmlui_app`/`bridge`/`render`, the vendored shader blobs (`pp/blobs/`, `pp/shaders/`, `agc_blobs.S`, `agc_ui_blobs.S`, `tools/build-shader.sh`). `evo_vdec_native` drops its `pp_agc` dependency. `libSceAgc`/`libSceAgcDriver` PRX stubs now populated from ps5-opengl's imports. Rewrote `gpu-notes.md`, `gpu-rendering-plan.md`, `agc-implementation.md`, `architecture.md`, `status.md`, `CLAUDE.md`. Closed `#67`/`#69`/`#70`/`#5`. **Kept** (user decision): both RmlUi render interfaces + the `EvoRenderBridge` seam — deleting `evo_rmlui_render.cpp` is a later story. (**Already done by GL-4:** GL as the only build, and the `#32` overlay machine.) | One dead-code-free render tree; `main.c` ~11,000 lines; docs match. |
...
| Issue | Change |
|---|---|
| `#67` (composite CPU text over GPU geo) | **CLOSED (GL-6, #82)** — the `#28` geo sink is deleted; GL renders text + geometry in one pass |
| `#68` (UI fidelity: curves, shadows, AA) | **Rescoped, open** — goals kept, mechanism is GLSL + RCSS; no longer blocked (GL is the render path); absorbs `#70` |
| `#69` (4× MSAA color surface + resolve) | **CLOSED (GL-6, #82)** — `EGL_SAMPLES` / multisample FBO + `glBlitFramebuffer` replaces the PA_SC register RE |
