...
| **GL-4** (#80) | Make the video path efficient. GL-3 B3 put the video on screen via GL but at a **~14 fps ceiling: `glTexSubImage2D` of a 1080p RGBA8 texture is ~68 ms on ps5-opengl** (synchronous CPU staging copy per upload). **Stage 1 (G47→G55 bump + `--gl-smoke` upload re-measure) done + hw-verified 2026-09-10:** the staging copy is **RGBA8-only** — R8 / RG8 uploads are free (1080p luma 0.10 ms, chroma 0.03 ms; 4K 0.90 / 0.46 ms). So an **NV12 two-plane upload (R8 + RG8) = 0.13 ms/frame (1080p) / 1.36 ms/frame (4K)** — real-time, **no zero-copy import needed**. Stage 2 = NV12 upload + GLSL YUV→RGB (BT.601 limited, `#62` parity) on the fullscreen quad + aspect math in the vertex quad (closes `#76`). Stage 3 = delete `pp_converter_fused/_parallel`, `pp_compute_pipeline`, `tile_copy`, the 5-way dispatch, the V8/V3/1080 backend enum. **Full plan: [gl4-video-path-plan.md](gl4-video-path-plan.md).** Handoff: [#80 comment](https://github.com/sainsaji/EVO-PLAYER-PS5/issues/80#issuecomment-5607427903). | GTA 4K + 1080p through the GL video path real-time; `#62` parity vs a reference frame; `#76` fixed; CPU converters + `tile_copy` + the backend enum gone. |
