...
OpenGL funnel — [evo-pro/opengl-render-overhaul.md](evo-pro/opengl-render-overhaul.md)).
...
| **#80** | `render-overhaul` GL-4 — video into the funnel: GLSL YUV→RGB + composite + one flip; delete the CPU converters (closes #76, resolves #62). **Plan: [evo-pro/gl4-video-path-plan.md](evo-pro/gl4-video-path-plan.md).** Stage 1 (ps5-opengl G47→G55 bump + `--gl-smoke` upload re-measure) done + hw-verified 2026-09-10 — the `glTexSubImage2D` wall is RGBA8-only, R8/RG8 free, so **no zero-copy needed**; Stage 2 = NV12 upload + GLSL YUV→RGB + aspect-in-vertex-quad; Stage 3 = converter demolition. | `projects/evoplayer/pp/src/` (`pp_converter_*.c`, `pp_compute_pipeline.c`, `tile_copy.c`, `pp_videoout.c`, `pp_playback.c`, `pp_agc.c` `agc_render_frame`), `main.c` (present dispatch, `prospero_apply_view_mode` ~L1605), `media/src/evo_vdec_native.c`, `ui_rml/src/evo_gl_context_device.cpp`, `pp/src/pp_gl_smoke.c`, `patches/ps5-opengl/` |
